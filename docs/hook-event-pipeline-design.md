...
audit.mjs
  ...
  └─→ fetch POST http://localhost:3000/api/hooks
        │
        ▼
  Next.js Route Handler (packages/web)
  POST /api/hooks
        │
...
### 5.2 packages/web — Next.js Route Handler

```
packages/web/
├── package.json          # next, react, react-dom, @cursor-consumption/core
├── tsconfig.json
├── next.config.ts
└── src/app/
    ├── layout.tsx
...
### 5.3 packages/core — DB保存関数

```
packages/core/src/storage/
├── queries.ts           # 既存（UsageEvent 用）
└── hook-queries.ts      # 新規: Hook Event 保存関数
```
...
audit.mjs (依存なし — Node.js 組み込み fetch)
  │
  ▼ HTTP POST
packages/web (next, react, react-dom, @cursor-consumption/core)
  │
  ▼ import
packages/core (prisma, @prisma/adapter-pg, pg)
  │
  ▼ SQL
PostgreSQL
```
...
```
1. pnpm --filter web dev  → Next.js 開発サーバー起動（localhost:3000）
...
1. `packages/core/src/storage/hook-queries.ts` — 保存関数を実装
2. `packages/core/src/index.ts` — hook-queries のエクスポートを追加
3. `packages/web/` — Next.js セットアップ（next, react, react-dom インストール）
4. `packages/web/src/app/api/hooks/route.ts` — Route Handler 実装
